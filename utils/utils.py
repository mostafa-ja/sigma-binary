...
def evaluate_adversarial_examples(
    ...
    indicator_masking: bool = False,
    rounded: bool = False,
    rounding_function=prioritized_binary_rounding,
    rounding_args=None,
    batch_size: int = 1024,
    norm: str = 'L0'
) -> torch.Tensor:
    ...
    # Print results
    print(f"\n{model.model_name}: Evaluation on Adversarial Examples, Norm: {norm}, Rounded: {rounded}, Rounding Function: {rounding_function.__name__ if callable(rounding_function) else rounding_function}")
    #print(f"\n{model.model_name} : Evaluation on Adversarial Examples (Norm: {norm}, Rounded: {rounded})")
    print("-----------------------------------------------")
