...
def CW(
    ...
    oblivion: bool = False,
    binary_search_steps_cw: int = 8,
    binary_search_steps_penalty: int = 4,
        ...
        for outer_step in range(outer_iters):
            ...
            for step in range(max_iterations):
                ...
                if detector_enabled and not oblivion:
                    dl_loss = CONST * torch.clamp(mal_score - ben_score + primary_confidence, min=0.0) + torch.clamp(prob - tau + secondary_confidence, min=0.0)
                else:
            ...
            CONST, upper_bound, lower_bound = update_bounds_and_consts(
                outer_step, binary_search_steps_penalty, CONST, upper_bound, lower_bound, prime_success,update_factor = 100.
            )
    ...
    output = thresholded_binary_rounding(o_bestadv)
