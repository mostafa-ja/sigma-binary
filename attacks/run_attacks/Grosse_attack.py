"""
Reproducible runner for the Grosse et al. (2017) binary-feature adversarial attack.

Usage example:
python -m attacks.run_attacks.Grosse_attack \
  --cuda --data-path dataset/malscan_preprocessed \
  --param-path defenses/saved_parameters \
  --model DNN --max-iterations 1000 --verbose
"""
...
from attacks.baseline_attacks.Grosse import Grosse


def main():
    parser = argparse.ArgumentParser(description="Run Grosse attack (reproducible, GitHub-ready).")
    ...
    parser.add_argument("--max-iterations", type=int, default=1000, help="Maximum iterations for Grosse attack")
    parser.add_argument("--binary-search-steps", type=int, default=4, help="Binary search steps (default 4)")
    ...
    with open(output_dir / "run_Grosse_metadata.json", "w", encoding="utf-8") as f:
    ...
    if file_paths["mals"].exists() and file_paths["advs"].exists():
        ...
        mals_adapt, advs_adapt = attack_mal_batches(
            test_loader, model, Grosse, feature_mask, device,
    ...
    print(f"[Grosse_attack] Finished. Results saved to {output_dir}")
