"""
Reproducible runner for the randomized Fast Gradient Sign Method (rFGSM) attack.

Usage example:
python -m attacks.run_attacks.rFGSM_attack \
  --cuda --data-path dataset/malscan_preprocessed \
  --param-path defenses/saved_parameters \
  --model DNN --max-iterations 1000 --step-length 0.01 --verbose
"""
...
from attacks.baseline_attacks.rFGSM import rFGSM


def main():
    parser = argparse.ArgumentParser(description="Run rFGSM attack (reproducible, GitHub-ready).")
    ...
    parser.add_argument("--batch-size", type=int, default=1024, help="Batch size for data loading")
    parser.add_argument("--verbose", action="store_true", help="Print attack progress")

    # Attack hyperparameters
    parser.add_argument("--max-iterations", type=int, default=5000, help="Maximum iterations for rFGSM attack")
    parser.add_argument("--step-length", type=float, default=0.01, help="Step length for each gradient update")
    parser.add_argument("--binary-search-steps", type=int, default=6, help="Binary search steps (default 6)")
    parser.add_argument("--initial-const", type=float, default=1.0, help="Initial constant for balancing loss terms")
    ...
    # Output directory and metadata logging
    ...
    with open(output_dir / "run_rFGSM_metadata.json", "w", encoding="utf-8") as f:
    ...
    if file_paths["mals"].exists() and file_paths["advs"].exists():
        ...
        mals_adapt, advs_adapt = attack_mal_batches(
            test_loader, model, rFGSM, feature_mask, device,
            detector_enabled=detector_enabled, oblivion=args.oblivion,
            indicator_masking=indicator_masking, verbose=args.verbose,
            max_iterations=args.max_iterations, binary_search_steps=args.binary_search_steps,
            initial_const=args.initial_const, step_length=args.step_length
        )
    ...
    print(f"[rFGSM_attack] Finished. Results saved to {output_dir}")
