"""
Reproducible runner for the Binary Gradient Ascent (BGA) attack
(Al-Dujaili et al., 2018)

Usage example:
python -m attacks.run_attacks.BGA_attack \
...
from attacks.baseline_attacks.BGA import BGA


def main():
    parser = argparse.ArgumentParser(description="Run BGA attack (reproducible, GitHub-ready).")
    ...
    parser.add_argument("--max-iterations", type=int, default=1000, help="Maximum iterations for BGA")
    ...
    with open(output_dir / "run_BGA_metadata.json", "w", encoding="utf-8") as f:
    ...
    # run attack or load cached results
    if file_paths["mals"].exists() and file_paths["advs"].exists():
        ...
        mals_adapt, advs_adapt = attack_mal_batches(
            test_loader, model, BGA, feature_mask, device,
    ...
    # evaluate generated adversarial examples
    ...
    print(f"[BGA_attack] Finished. Results saved to {output_dir}")
