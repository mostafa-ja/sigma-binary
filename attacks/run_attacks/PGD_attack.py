...
def main():
    parser = argparse.ArgumentParser(description="Run PGD attack on malware detection models.")
    ...
    parser.add_argument("--max-iterations", type=int, default=100, help="Maximum iterations for PGD attack")
    parser.add_argument("--step_length", type=float, required=True, help="step length for the attack")
    ...
    if os.path.exists(file_paths['mals']) and os.path.exists(file_paths['advs']):
        ...
        mals_adapt, advs_adapt = attack_mal_batches(
            ...
            norm=args.norm, binary_search_steps=args.binary_search_steps, 
