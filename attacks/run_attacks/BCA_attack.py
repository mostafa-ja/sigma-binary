"""
Reproducible runner for the Binary Coordinate Ascent (BCA) adversarial attack.

Usage example:
python -m attacks.run_attacks.BCA_attack \
...
from attacks.baseline_attacks.BCA import BCA


def main():
    parser = argparse.ArgumentParser(description="Run BCA attack (reproducible, GitHub-ready).")
    ...
    parser.add_argument("--max-iterations", type=int, default=1000, help="Max iterations for BCA")
    parser.add_argument("--binary-search-steps", type=int, default=6, help="Binary search steps")
    parser.add_argument("--initial-const", type=float, default=1.0, help="Initial constant for balancing")
    ...
    with open(output_dir / "run_BCA_metadata.json", "w", encoding="utf-8") as f:
    ...
    if file_paths["mals"].exists() and file_paths["advs"].exists():
        ...
        mals_adapt, advs_adapt = attack_mal_batches(
            test_loader, model, BCA, feature_mask, device,
    ...
    print(f"[BCA_attack] Finished. Results saved to {output_dir}")
